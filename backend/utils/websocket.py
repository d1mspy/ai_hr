...
from llm_interviewer.utils.state_classes import TopicType
import logging
...
from .handling_llm import llm
from .voice.voice_models import silero_tts_model
...
voice_generator = VoiceGenerator(silero_tts_model)
    ...
    async def send_message(self, user_id: int, message: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"send_text retry failed for user {user_id}: {e2}")
...
class AudioConnectionManager(ConnectionManager):
    ...
    def set(self):
        llm.vacancy_general = self.data.summary
        llm.resume_context = self.data.meta
        llm.state = [{"name": "Приветствие", "type": TopicType.GREETING},
                     {"name": "Информация о вакансии", "type": TopicType.VACANCY_INFO}] + self.data.soft_topics + self.data.hard_topics
    ...
    async def handle_audio_chunk(self, user_id: int, data: dict):
        ...
        if not self.is_session_active:
            await self.send_message(user_id, "No active session")

            return

        if user_id != self.active_user_id:
            await self.send_message(user_id, "Not authorized for this session")
            print("wrong user_id")
            return
        ...
        try:
            ...
            result = self.chunk_processor(audio_float)
            self.chunks_processed += 1

            await self.handle_model_result(user_id, result)

        except Exception as e:
            await self.send_message(user_id, f"Processing error: {e}")

    async def handle_model_result(self, user_id: int, result: ChunkProcessAns):
        """Обработка результата от ChunkProcessor"""
        # print(self.current_interview_text)
        # print(self.pending_user_response)
        # print(result.status)
        if result.status == 'ok':
            ...
            if text:
                self.awaiting_user_response = False  # чтобы не было реэнтри
                try:
                    llm_response = llm.process_message(text)
            ...
            await self.send_message(user_id, f"Model error: {result.content}")
    ...
    async def send_llm_response_in_audio(self, user_id: int, response_text: str):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to generate/send audio for user {user_id}: {e}")
            return False
    ...
    async def handle_audio_end(self, user_id: int, data: dict):
        ...
        text = self.pending_user_response.strip()
        if text and self.process_text_callback:
            # гарантируем, что обработаем хвост даже если флаг был False
            self.awaiting_user_response = False
            try:
                ...
                await self.send_message(user_id, json.dumps({
                    ...
                    'current_topic': getattr(llm_response, "current_topic", None),
                    'timestamp': datetime.now().timestamp()
                }))
                await self.send_llm_response_in_audio(user_id, llm_response.text)
            finally:
    ...
    async def force_end_session(self):
        ...
        if self.is_session_active:
            ...
            self.logger.info(
                f"Session ended. Chunks: {self.chunks_processed}, Duration: {duration:.1f}s")
